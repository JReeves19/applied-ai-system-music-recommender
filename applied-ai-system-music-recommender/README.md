...
1. **Understand** — Claude (`claude-sonnet-5`) parses a free-text request into a
...
The retrieved songs are what Claude reasons over — the AI actively uses the
retrieved data to form its answer, rather than printing it alongside a canned
response.

**Guardrails, logging, and reliability:**
- If `ANTHROPIC_API_KEY` is missing or an API call fails, the app **degrades
...
- **Anti-hallucination check:** any catalog song Claude names that wasn't
...
```
data/songs.csv                 # 18-song catalog (the knowledge base)
...
tests/test_rag.py              # RAG pipeline with a mocked Anthropic client
```
...
3. Set your Anthropic API key (required only for the RAG path; the app still
   runs without it). Easiest option — copy the example env file and fill it in;
   the app loads it automatically via python-dotenv:
...
**Tests** (no API key needed — the Anthropic client is mocked):
